...
@Command({
    name: '시장',
    description: '서버 기준으로 현재 시장에 등록되어 있는 특정 아이템의 목록을 조회합니다. (글로벌 전용)',
})
@UsePipes(TransformPipe)
export class MarketCommand implements DiscordTransformedCommand<MarketSearchDto> {
    ...
    async handler(
        ...
        try {
            const embedMsg = await this.marketService.getInfo(dto.server, dto.keyword);

            await interaction.editReply({embeds: [embedMsg]});
        }
        catch (e) {
            if (e instanceof MarketError || e instanceof ItemSearchError || e instanceof ItemSearchTooManyResultsError) {
