...
import { AggregatedItemInfo, ItemSearchService } from '../item/item-search.service';
...
export class MarketService {
    ...
    async getInfo(server: string, keyword: string) {
        ...
        const marketRes = await this.universalisService.fetchCurrentList(server, itemId);
        if (!marketRes.data.hasOwnProperty('listings') || marketRes.data.listings.length < 1) {
            throw new MarketError('시장에 등록된 아이템이 없어요.');
        }

        const total = marketRes.data.listings.length;
        let itemStr = this.makeItemInfo(total, marketRes);
        let historyStr = this.makeItemHistoryInfo(marketRes);
    ...
    private makeMessage(filtered: AggregatedItemInfo, itemId: number, list: AxiosResponse<any>, itemStr: string, historyStr: string) {
        const total = list.data.listings.length;
        const historyTotal = list.data.recentHistory.length;
        const lastUploadTime = moment(new Date(list.data.lastUploadTime)).format('YYYY년 MM월 DD일 HH시 mm분 ss초');
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: `최소 (NQ/HQ)`,
                    value: `${list.data.minPriceNQ.toFixed(2).toLocaleString()}길 / ${list.data.minPriceHQ.toFixed(2).toLocaleString()}길`,
                    inline: true
                },
                {
                    name: `최대 (NQ/HQ)`,
                    value: `${list.data.maxPriceNQ.toFixed(2).toLocaleString()}길 / ${list.data.maxPriceHQ.toFixed(2).toLocaleString()}길`,
                    inline: true
                },
                {
                    name: `시장 검색 결과 (총 ${total >= 100 ? '100건 이상' : `${total.toLocaleString()}건`})`,
                    value: itemStr
                },
                {
                    name: `최근 구매 내역 (총 ${historyTotal >= 100 ? '100건 이상' : `${historyTotal.toLocaleString()}건`})`,
                    value: historyStr
                },
                {
                    name: '마지막 갱신 날짜',
