...
export class NoticeCreatePostCollector {
    ...
    async onCollect(interaction: SelectMenuInteraction): Promise<void> {
        ...
        try {
            ...
            await this.addUrl(locale, topic, hookUrl);
            this.loggerService.log(`${interaction.guild} (${interaction.guildId}) - 언어: ${locale}, 카테고리: ${topic} - 소식을 추가하였습니다.`);
     ...
     */
    private async addUrl(locale: string, type: string, url: string) {
