...
export class ChatGptService {
    ...
    async converse(guildId: string, userId: string, message: string) {
        ...
        if (locked && locked === '1') return '으음... 다른 달달이랑 이야기하고 있는 것 같아요. 잠시만 기다려주세요!';
        ...
        try {
            ...
            if (errorMsg.indexOf('token') > -1) {
                ...
                await GlobalErrorReport.report('error', 'GPT-3 대화 오류가 발생했습니다.', e);
            }
        }
        await this.setConverseLock(guildId, userId, 0);
    ...
    private async setConverseLock(guildId: string, userId: string, value: 0 | 1) {
        return this.redis.set(`openai-converse-lock-${guildId}-${userId}`, value, 'EX', 600);
    }
    ...
    private async setConverseHistory(guildId: string, userId: string, message: string) {
        return this.redis.set(`openai-converse-${guildId}-${userId}`, message, 'EX', 600);
    }
}
