...
export class ItemStoreService {
    ...
    private async getLatestKoreanVersionFromDB() {
        return await this.xivVersionRepository.findOne({ where: [{ locale: 'kr' }], order: { version: 'DESC' } });
    }

    private async getLatestKoreanVersionFromRemote() {
        const tags = await this.remoteGitTags('https://github.com/Ra-Workspace/ffxiv-datamining-ko');
        return [...tags].map(i => parseFloat(i.at(0).replace('v', ''))).sort((a, b) => {
            return b - a;
        }).reverse().pop();
    }
    ...
    private async fetchCsv(fileName: string) {
        return axios.get(`https://raw.githubusercontent.com/Ra-Workspace/ffxiv-datamining-ko/master/csv/${fileName}.csv`);
    }
    ...
    private readCsv(content: string) {
        ...
        let lines = [];
        let line = [];
        for (let strIdx = 0, strTotal = content.length; strIdx < strTotal; strIdx++) {
