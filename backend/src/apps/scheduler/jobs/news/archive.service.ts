...
export class ArchiveService {
     ...
     */
    async getGlobal(type: NewsCategoryGlobal, locale: string, isSkipCache: boolean = false): Promise<NewsContent[]> {
        let outdate = await this.isOutDate(type, locale);
        if (isSkipCache || outdate) {
            try {
                let data = await ArchiveFetchHelper.withGlobal(NewsCategories.Global[type].url, type, locale);
                await this.setCache(JSON.stringify(data), type, locale);
                return data;
            } catch (e) {
                this.logMessage('warn', '글로벌 소식을 가져오는 과정에서 오류가 발생했습니다.', e);
    ...
    async getGlobalAll(locale: string): Promise<any[]> {
        ...
        for (let idx in NewsCategoryGlobal) {
            results.push(await this.getGlobal(NewsCategoryGlobal[idx as keyof typeof NewsCategoryGlobal], locale));
        }
        return results;
    }
     ...
     * @param isSkipCache Cache 사용 여부
     */
    async getKorea(type: NewsCategoryKorea, isSkipCache: boolean = false): Promise<NewsContent[]> {
        let outdate = await this.isOutDate(type, 'kr');
        if (isSkipCache || outdate) {
            try {
                let data = await ArchiveFetchHelper.withKorea(NewsCategories.Korea[type].url, type);
                await this.setCache(JSON.stringify(data), type, 'kr');
                return data;
            } catch (e) {
                this.logMessage('warn', '한국 소식을 가져오는 과정에서 오류가 발생했습니다.', e);
    ...
    async getKoreaAll(): Promise<any[]> {
        ...
        for (let idx in NewsCategoryKorea) {
            results.push(await this.getKorea(NewsCategoryKorea[idx as keyof typeof NewsCategoryKorea]));
        }
        return results;
    }
