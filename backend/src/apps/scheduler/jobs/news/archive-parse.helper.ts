...
export class ArchiveParseHelper {
    ...
    if ($list && $list.length > 0) {
      $list.each(function (idx, data) {
        ...
        const parseDetail: NewsContent = {
          ...
          title: $(this)
            .find('p.news__list--title')
            .text()
            .replace(/(\[.*\])|(\r\n|\n|\r)/gm, '')
            .trim(),
          timestamp:
            parseInt(_script.match(/ldst_strftime\((\d+)./)?.at(1) || '') *
            1000,
        };
      ...
      $list.each(function (idx, data) {
        ...
        $content('p')
          ...
          .each(function (idx, ele) {
            if ($(this).text().length > 0) descs.push($(this).text());
          });
   ...
   * @param pLocale 언어
   */
  static async parseGlobalMaintenance(
    $: CheerioAPI,
    pLocaleBaseUrl: string,
    pLocale: string,
  ): Promise<NewsContent[]> {
    ...
    if ($list && $list.length > 0) {
      $list.each(function (idx, data) {
        ...
        const parseDetail: NewsContent = {
          ...
          url: `${pLocaleBaseUrl}${$title.find('a').attr('href')}`,
          thumbnail:
            'http://static.ff14.co.kr/Contents/2019/04/0B93217EE978FE3F5AFFD847A20A55D20FF200821CBB6124AFDFEC38384E2FC8.jpg',
        };
  ...
  static async parseKoreaMaintenance(
    $: CheerioAPI,
    pLocaleBaseUrl: string,
  ): Promise<NewsContent[]> {
    let list: NewsContent[] = [];
    ...
    if ($list && $list.length > 0) {
      $list.each(function (idx, data) {
        ...
        const parseDetail: NewsContent = {
          ...
          url: `${pLocaleBaseUrl}${$title.find('a').attr('href')}`,
          thumbnail:
            'http://static.ff14.co.kr/Contents/2019/07/97809A6EB08E63368C57F973277459AD7AC75C71426E2D0B0613FE636FA63706.jpg',
        };
    ...
    if ($list && $list.length > 0) {
      $list.each(function (idx, data) {
        ...
        const parseDetail: NewsContent = {
          ...
          url: `${pLocaleBaseUrl}${$title.find('a').attr('href')}`,
          thumbnail:
            'http://static.ff14.co.kr/Contents/2015/10/2015103017255463465.jpg',
        };
    ...
    if ($list && $list.length > 0) {
      $list.each(function (idx, data) {
        ...
        const parseDetail: NewsContent = {
          ...
          title: ($(this).find('span.txt').html() || '')
            .replace(/([\r\n|\n|\r])/gm, '')
            .trim(),
          url: `${pLocaleBaseUrl}${url}`,
          thumbnail: `https:${_thumbnail[1]}`,
          summary: ($(this).find('.summary.dot').html() || '')
            .replace(/([\r\n|\n|\r])/gm, '')
            .trim(),
        };
